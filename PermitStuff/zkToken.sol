/*CHANGE TO SHOULD BE BEFORE LAUNCH
	Change pools from 2 pools to 1 for testing
...
contract MTK_Token is ERC20Permit, Ownable {
...
// Managment events
    ...
    uint public miningTarget = _MAXIMUM_TARGET.div(250);  //1000 million difficulty to start until i enable mining
	...
	function ARewardSender() public {
		...
		if(rewardEra < 8){
			...
			reward_amount = uint( 20 * 10**18).div( 2**(rewardEra - 7  ) );
		}
	...
	function getMiningReward() public view returns (uint) {
		...
		if(rewardEra < 8){
			...
			return uint( 20 * 10**(18)).div( 2**(rewardEra - 7  ) );
		}
	}
